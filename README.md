...
- непрерывный диалог с передачей предыдущих сообщений в контекст модели;
- локальное хранение до 20 диалогов (по 30 последних ходов) между перезагрузками страницы;
- новый независимый набор эффектов для каждого сообщения;
...
Диалоги и выпавшие эффекты сохраняются в хранилище текущего браузера. API-ключ отправляется только в backend во время генерации и не сохраняется.
